...
$('#addUserBtn').onclick = async () => {
  ...
  if (!isFullAccess() && !userLocationIds().includes(locationId)) return toast('You can only add users to your locations');
  try {
    await saveUser({ name, email, role, locationId, locationIds: [locationId], invitedBy: currentUser().name });
    $('#newUserName').value = '';
    $('#newUserEmail').value = '';
